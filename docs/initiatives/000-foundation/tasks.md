...
| `T022` | 进行中 | 已补首版治理工作台单页，接通内容导入、来源创建、审核决策、知识发布、策略灰度与问答诊断，但仍缺鉴权、分页筛选与回滚发布。 |
| `T023` | 进行中 | 已补小程序主对话页、分享承接页、最近会话/依据/反馈入口，但流式回答仍是首版兼容实现，且当前工程缺 Taro CLI 依赖，尚未完成构建验证。 |
| `T024` | 进行中 | 已补 analytics 事件 schema、ai-orchestrator 本地评测集与评分脚本，但前后端真实埋点接入和线上评测闭环仍未建立。 |
| `T025` | 进行中 | 已补 observability compose、Prometheus/Loki/OTel 配置与基础 runbook，但业务服务真实指标、告警和 dashboard 仍未落地。 |
| `T026` | 进行中 | 已补首发验收清单、发布计划与已知问题文档，但正式首发验收和环境演练仍未完成。 |
