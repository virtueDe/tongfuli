...
| `T010` | 未开始 | 仍缺公共/后台 API 契约测试目录与样例。 |
| `T011` | 未开始 | 事件契约与导入格式测试尚未建立。 |
| `T012` | 未开始 | 内容与来源治理域模型仍未真正实现。 |
| `T013` | 进行中 | 会话域已落地，策略域仍明显缺失。 |
| `T014` | 未开始 | 导入、采集与抽取管线尚未进入实现。 |
| `T015` | 未开始 | 审核、发布与索引刷新仍是占位。 |
| `T016` | 进行中 | 已有内部回答接口，但多路检索与真实编排未完成。 |
| `T017` | 未开始 | 后校验、降级、风控与限流尚未建立。 |
| `T018` | 进行中 | 已补齐公共会话首版接口：建会话、流式回答、角色切换、模式切换、依据展开、最近会话、反馈；但契约测试、持久化与风控未补。 |
| `T019` | 未开始 | 后台内容/来源/策略/诊断 API 仍未实现。 |
| `T020` | 进行中 | Web 主对话页已接通角色切换、模式切换、依据展开；专题与分享链路仍在后续任务。 |
| `T021` | 未开始 | Web 专题页与分享落地页尚未开始。 |
